...
地图展示使用 Leaflet 和 OpenStreetMap 瓦片。地点搜索优先走本地数据服务的 `/api/place-search` 代理，代理使用 Photon 搜索；分享网站没有本地服务时会直接调用 Photon。这个方案适合低频个人规划；如果以后做成多人高频公开产品，需要换成正式地图 / 地理编码服务并配置自己的服务额度。
